...
class CircularQueue:
    """FIFO circular queue implementation using a singly linked list for storage."""
    ...
    def rotate(self):
        """Move element at the front to the back of the queue"""
        if self.is_empty():
            raise Empty('Stack is empty')
        ...
        myback = self._tail
        self._head = self._head.next_node
        myback.next_node = myfirst
        myfirst.next_node = None
        self._tail = myfirst
